...
class XY_Tokenizer(nn.Module):
    def __init__(self, generator_params):
        ...
        self.vocos = Vocos(**generator_params['vocos_kwargs'])
    ...
    def inference_detokenize(self, codes, codes_lengths):
        ...
        y, vocos_output_length = self.vocos(acoustic_decoder_output, acoustic_decoder_output_length) # (B, 1, T), 100hz -> 16khz
    ...
    def encode(self, wav_list, overlap_seconds=10, device=torch.device("cuda")):
        ...
        chunk_size = int(30 * self.sample_rate) # Maximum samples per chunk
        duration_size = int(duration_seconds * self.sample_rate) # Valid output samples per chunk
        code_duration_length = duration_size // self.downsample_rate # Valid code length per chunk
        ...
        if codes_list:
            ...
            codes_list = [codes_tensor[:, i, :input_lengths[i] // self.downsample_rate] for i in range(batch_size)] # B * (nq, T)
        else:
    ...
    def decode(self, codes_list, overlap_seconds=10, device=torch.device("cuda")):
        ...
        """
        duration_seconds = 30 - overlap_seconds
        chunk_code_length = int(30 * self.sample_rate // self.downsample_rate) # Maximum code length per chunk
        duration_code_length = int(duration_seconds * self.sample_rate // self.downsample_rate) # Valid code length per chunk
        duration_wav_length = duration_code_length * self.downsample_rate # Valid waveform length per chunk
        ...
        if wav_list:
            ...
            syn_wav_list = [wav_tensor[i, 0, :code_lengths[i] * self.downsample_rate] for i in range(batch_size)] # B * (T,)
        else:
